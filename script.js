            ...
            return `
                <div class="video-card" data-index="${index}">
                    <div class="video-thumbnail">
                        <img src="${loadingThumbnail}" alt="Thumbnail" class="thumbnail-img" data-video-url="http://localhost:3000${video.path}">
...
async function deleteVideo(filename) {
    if (confirm('Deseja realmente remover este vídeo?')) {
        try {
            const response = await fetch(`http://localhost:3000/api/videos/${filename}`, {
                method: 'DELETE'
            });
...
async function openModal(video) {
    ...
    const videoUrl = `http://localhost:3000${video.path}`;
